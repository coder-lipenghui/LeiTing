...
第二关终极 Boss `boss_02` 使用 3 个独立阶段：100%-70% 血量时中心发射每轮
5 圈的圆形弹幕，左右发射口每 2 秒各发射 3 枚中速追踪导弹；70%-40% 血量时
中心发射大风车弹幕，左右发射口每 1 秒各发射 2 枚锁定高速导弹；40%-0% 血量
时按“黄色锁定激光、三口圆形弹幕、三口高速追踪导弹、三口高速锁定导弹”的
顺序循环。第三阶段的 `attackInterval` 为 1.25 秒，完整四招循环为 5 秒，激光
本身的 `lifetime` 为 2 秒。

`BossPhasePattern.patternId` 带 `@间隔+首次偏移` 时，该模式按自己的时间表独立
循环；同一阶段的模式都不带 `@` 时，则按照 `sort` 顺序逐项循环。这两种方式
分别用于 `boss_02` 的前两个阶段和第三阶段。
