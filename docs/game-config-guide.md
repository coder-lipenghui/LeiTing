...
携带 `Bomb`、`Heal`、`Shield`、`Magnet` 或 `WeaponUp` 类型道具的普通敌机会显示与场景特殊掉落物一致的静态青色柔光。第二关开场的 7 个资源机波次中，每局随机选择 3 个波次；每个入选波次再随机选择 1 架，额外携带并掉落 `weaponup`，合计 3 个攻击力提升道具。
...
Boss 阶段配置实现 50% 血量切换，但属于中间首领；击破后关卡时间轴继续运行，
180 秒仍会生成终极 Boss `boss_02`。该中间首领不显示 Boss 血条、入场文字或
...
的大风车弹幕，并且仅左右发射口发射高速追踪导弹；40%-0% 血量时取消圆形
