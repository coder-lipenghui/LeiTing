...
携带 `Bomb`、`Heal`、`Shield`、`Magnet` 或 `WeaponUp` 类型道具的普通敌机会显示与场景特殊掉落物一致的静态青色柔光。第二关开场资源机的攻击力提升道具由 `WaveSpawn.dropItemId` 和 `dropIndex` 固定配置，不会在运行时重新随机；当前配置共生成 3 个 `weaponup`。

第二关在 38 秒从右侧依次生成 8 架 `enemy_small_04`，沿左下方向飞行；42 秒再从左侧依次生成 8 架相同飞机，沿右下方向飞行。两组飞机入场后均按 `enemy_single_down` 模式发射单发子弹。
...
作用：定义可拾取物的表现和收益。星星和金币增加玩家资源；磁体、炸弹、红心、盾牌、攻击力提升通过 `itemType` 触发对应效果。

| 字段 | 类型 | 作用 | 示例 |
| --- | --- | --- | --- |
| `id` | string | 掉落物唯一 ID。 | `"star"` |
| `displayName` | string | 显示名称。 | `"星星"` |
| `itemType` | string | 掉落物类型，支持 `Star`、`Coin`、`Magnet`、`Bomb`、`Heal`、`Shield`、`WeaponUp`；`WeaponUp` 每次拾取使玩家本局子弹伤害增加 1。 | `"Star"` |
...
作用：按关卡时间生成敌机组。每个波次包含一个或多个 `spawns`，按数组顺序执行；同一 `WaveSpawn` 的 `count` 大于 1 时，敌机会按 `interval` 逐架出生。

运行时同时维护“总战斗时间”和“普通刷怪内容时间”。Boss 出现后普通刷怪内容时间暂停，
已经启动但尚未刷完的普通敌机组也会等待 Boss 退场，因此不会在 Boss 战中继续堆怪；
最后一个 Boss 波次按总战斗时间判断是否到点，但必须等待当前 Boss、Boss 交接过渡以及
它之前的必要普通波次开始并完成生成。Boss 出场前会清除遗留的敌方子弹和导弹。
...
| `dropItemId` | string | 本刷怪组中指定敌机额外掉落的道具 ID；留空表示不配置。 | `"weaponup"` |
| `dropIndex` | int | 携带掉落物的组内敌机序号，从 1 开始；`0` 表示无。 | `2` |
...
作用：按总战斗时间触发一次性事件，目前支持显示提示文本、清除敌方子弹和生成奖杯道具。
它们不会因为 Boss 暂停普通刷怪时间线而延后。
...
第二关在 105 秒生成 `boss_level_02_mid_01`，资源为 `enemy_09.prefab`。它通过
Boss 阶段配置实现 50% 血量切换，但属于中间首领；战斗期间总战斗时间和
119/136/153/170 秒的奖杯事件继续运行，普通刷怪内容时间暂停。终极 Boss
`boss_02` 在总战斗时间 180 秒到点：如果中间首领仍存活就进入等待队列，待其
死亡演出结束后再保留 1.25 秒交接时间，随后清弹并生成；如果中间首领提前击破，
则仍等到 180 秒。该中间首领不显示 Boss 血条、入场文字或
阶段切换文字。左右导弹模式使用 `left,left1,right,right1` 四个独立挂点，每个
挂点发射 1 枚导弹，确保左右各两枚导弹不会在生成点重叠。

第二关终极 Boss `boss_02` 使用 `Assets/Prefabs/Enemies/BOSS_2.prefab` 形象和
500 点生命，并分为 3 个独立阶段：100%-70% 血量时中心每 2 秒发射一轮 3 圈
圆形弹幕，左右发射口每 2 秒各发射 1 枚追踪导弹；70%-40% 血量时中心发射速度降为 `2.3`
的普通红粉色敌弹组成大风车，不再使用旋转/摇摆米粒弹，并且仅左右发射口发射
高速追踪导弹。大风车每次发射 24 轮、
每轮 6 发、轮间隔 0.08 秒、旋转步进 11 度；发射期间 Boss 按弹幕实际持续时间
停止移动，避免连续发射原点漂移破坏螺旋躲避通道。风车期间到点的高速追踪
导弹会延后到 Boss 恢复移动 0.25 秒后再发射，避免封死风车通道。40%-0% 血量时取消圆形
弹幕，外侧 `left1`、`right1` 发射口先用细红线追踪玩家 2 秒，随后冻结锁定方向
并蓄能 0.5 秒，最后发射持续 0.8 秒的激光。激光持续期间 Boss 停止移动。激光
结束后，中心发射口每 3 秒发射一轮 3 枚追踪导弹，单轮内每枚间隔 0.5 秒；
激光会沿实际发射方向动态延伸至屏幕外，确保画面内不会显示被截断的光束端点。
完整激光锁定序列每 5 秒再次触发。

第三阶段激光使用白色或浅黄色高亮核心、红橙色外层光晕，并通过着色器在光束
边缘生成轻微能量波动。

所有非激光敌机子弹都会显示光晕；即使 `glowRange` 未配置，运行时也会按弹体
尺寸提供最小光晕范围。当前光晕颜色按源图片主色校准：普通敌弹为红粉色、米粒
弹为洋红色、第二关直升机弹为紫色，并分别配置不同的透明度和扩散范围。

`BossPhasePattern.patternId` 带 `@间隔+首次偏移` 时，该模式按自己的时间表独立
循环；同一阶段的模式都不带 `@` 时，则按照 `sort` 顺序逐项循环。`boss_02`
第三阶段用独立的 5 秒和 3 秒排程分别触发激光锁定序列与中心追踪导弹。
